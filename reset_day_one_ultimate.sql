-- 🚨 SCRIPT DE LIMPEZA TOTAL ("DIA 1" - VERSÃO DEFINITIVA) 🚨
-- Apaga TUDO para deixar o site como novo.

-- 1. DESATIVAR TRAVAS (Para apagar sem erros)
SET session_replication_role = 'replica';

-- 2. LIMPAR VENDAS E LIVES (Geral)
TRUNCATE TABLE order_items, orders CASCADE;
TRUNCATE TABLE live_cart_items, live_carts, live_cart_status_history CASCADE;
TRUNCATE TABLE live_events, live_comments, live_pendencias, live_charge_logs CASCADE;
TRUNCATE TABLE live_customers, live_raffles, live_waitlist CASCADE;
TRUNCATE TABLE payments, mp_payment_events CASCADE;
TRUNCATE TABLE coupon_uses CASCADE; -- Preservando "coupons" (configuração) se quiser, ou apague aqui.
-- TRUNCATE TABLE coupons CASCADE;  -- Descomente se quiser apagar os CUPONS CADASTRADOS.
