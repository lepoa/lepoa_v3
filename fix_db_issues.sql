...
-- ===============================================================
-- SCRIPT DE CORREÇÃO PARA ERRO "text[] -> integer"
-- ===============================================================

-- 1. Tenta corrigir o tipo da coluna 'variante' em 'live_cart_items' para JSONB
-- Isto e necessario porque o operador JSON (->) nao funciona em colunas de Array de Texto (text[])
DO $$
BEGIN
  -- Se for text[], converte para JSONB
  BEGIN
    ALTER TABLE public.live_cart_items 
    ALTER COLUMN variante TYPE JSONB USING variante::jsonb;
  EXCEPTION WHEN OTHERS THEN
    -- Se falhar a conversao direta, ignora erro se ja for jsonb
    NULL;
  END;
END $$;

-- 2. Tenta corrigir o tipo da coluna 'shipping_address_snapshot' em 'live_carts'
DO $$
BEGIN
  BEGIN
    ALTER TABLE public.live_carts 
    ALTER COLUMN shipping_address_snapshot TYPE JSONB USING shipping_address_snapshot::jsonb;
  EXCEPTION WHEN OTHERS THEN
    NULL;
  END;
END $$;

-- 3. Recria a funcao de sincronizacao para garantir que esta usando os operadores corretos
-- Esta funcao e chamada pelo Trigger quando o status muda para 'aguardando_pagamento'
  ...
  IF v_existing_order_id IS NOT NULL THEN
    ...
    UPDATE orders SET
      status = CASE v_cart.status
        WHEN 'pago' THEN 'pago' -- Simplified logic for fallback
